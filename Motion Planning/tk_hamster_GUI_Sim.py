...
class virtual_world:
    def __init__(self):
        ...
        self.area = [] #self.vworld.area = [-300,-200,300,200] in my_sim
        self.map = []
        self.cobs = []
        self.f_cell_list = []
        self.goal_list = []
    ...
    def tempDraw(self, points, color):
        if color == 'blue':
            print("Got blue call in tempDraw")
            print(len(points))
        print(points)
        for point in points:
            time.sleep(0.01)
            ...
            if color == 'blue':
                r = 2
            self.canvas.create_oval(self.canvas_width + point[0]-r,self.canvas_height - point[1]-r,self.canvas_width + point[0]+r,self.canvas_height - point[1]+r, outline=color, fill=color)

    def tempDrawLine(self, lines, color):
        for line in lines:
            time.sleep(0.01)
            self.canvas.create_line(self.canvas_width + line[0], self.canvas_height - line[1], self.canvas_width + line[2], self.canvas_height - line[3], fill=color, width=5)

    def tempDrawRect(self, rects, color):
        for rect in rects:
            time.sleep(0.01)
            # print(rect[0])
            self.canvas.create_rectangle(self.canvas_width + rect[0], self.canvas_height - rect[1], self.canvas_width + rect[2], self.canvas_height - rect[3], outline=color, width = 3)
    ...
    def draw_map(self):
        ...
        for cobs in self.cobs:
            ...
            self.canvas.create_rectangle([x1,y1,x2,y2], fill="light grey")
