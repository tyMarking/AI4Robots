...
class BehaviorThreads(object):
    ...
    def __init__(self, robot_list):
        ...
        ###################################
        """
        States: moving, avoiding, border
        Events: obstacle, free, border
        """
        diagram = {}
        #add diagram parts
        addPath(diagram, "moving", "free", self.moveForward, "moving")
        addPath(diagram, "moving", "obstacle", self.turn, "avoiding")
        addPath(diagram, "moving", "border", self.get_out, "border")
        addPath(diagram, "avoiding", "free", self.moveForward, "moving")
        addPath(diagram, "avoiding", "obstacle", self.dummyFunc1Arg, "avoiding")
        addPath(diagram, "avoiding", "border", self.get_out, "border")
        addPath(diagram, "border", "free", self.moveForward, "moving")
        addPath(diagram, "border", "obstacle", self.turn, "avoiding")
        addPath(diagram, "border", "border", self.dummyFunc, "border")


        fsm = StateMachine(diagram, self.motion_q)

        t_motion_handler = threading.Thread(name='motion FSM thread', target=fsm.threadedRun, args=["moving"])
    ...
    def turn(self, direction):
        if direction == "right":
            self.robot_list[0].set_wheel(0,10)
            self.robot_list[0].set_wheel(1,-30)
        elif direction == "left":
            self.robot_list[0].set_wheel(0,-30)
            self.robot_list[0].set_wheel(1,10)
        else:
    ...
    def get_out (self):
        ...
        time.sleep(0.2)
        self.robot_list[0].set_musical_note(60)
        time.sleep(0.2)
